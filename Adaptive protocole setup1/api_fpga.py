...
#        
        ...
        while condition==True:
            # Verify if too much time elapsed
            if time.time()-t0>0.25:
                # Call a dummy metho to be overid. 
                self.dummy_event_pulse_is_long()
                t0 = time.time() # Important to restart the reference time
...
#        print("Element remaing: ", num_elems)        
    ...
    def dummy_event_pulse_is_long(self):
        """
        Dummy function to be overif by a higher level objetc that uses this 
        class.
        This method is called whenerver too much time elapsed in the while loop
        in the mehtod "run_pulse". 
        For example, this method can be overid by a "self.process_events()"
        in a gui that runs the pulse sequence. This should help to avoid freeze
        out of the gui
        """
        _debug('FPGA_api: run_pulse_loop: dummy_event_pulse_is_long')
