...
def mean_and_std_2D(x,y, prob):
    """
    Compute the means and the standard deviation of xs and ys according to the probability 
    ...
    # Than compute the standard deviation
    std_x  = np.sqrt( integral_2D(x, y, prob*(mesh_x - mean_x)**2) )
    std_y  = np.sqrt( integral_2D(x, y, prob*(mesh_y - mean_y)**2) )
...
class GUIBayesAnalyser(egg.gui.Window):
    ...
    def __init__(self, name="Bayes inference T1", size=[1000,500]): 
        ...
        self.std_gp, self.std_gm       = (0, 0) # Best estimate of the std  so far
    ...
    def label_info_update(self):
        ...
        txt = ('Best rates G+ so far= %f +- %f kHz'%(self.best_gp*1e-3, self.std_gp*1e-3)
            +'\nBest rates G- so far= %f +- %f kHz'%(self.best_gm*1e-3, self.std_gm*1e-3)
    ...
    def determine_time_to_probe(self, gp_axis, gm_axis, post):
        ...
        if self.method == 'Mean_rate':    
            ...
            res = mean_and_std_2D(gp_axis, gm_axis, post)
            self.best_gp, self.best_gm, self.std_gp, self.std_gm = res
